...
backpacks = backpacks.strip().splitlines()

alfabet = alfabet.lower().split(' ')
...
totalPriority = 0;
for groups in chunks(backpacks, 3):
    existingBadges = [];
    for character in list(list(groups)[0]):
        if character in list(groups)[1] and character in list(groups)[2]:
            if character not in list(existingBadges):
                existingBadges.append(character)
